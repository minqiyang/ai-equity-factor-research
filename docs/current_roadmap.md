...
| 4a. Experiment/trial ledger contract | Local gates passed; PR, CI, final-review, protected-merge, and exact merge-head CI gates pending | Freeze semantic trial versus attempt identity, preallocation, lifecycle, campaign inventory/accounting closure and adjudication, protected access, canonical request/event chain and campaign evidence-prefix checkpoint, review binding, and exact private/public projection contracts without selecting a backend. | `docs/experiment_trial_ledger_contract.md`, its synthetic golden vectors, all `LEDGER-*` documentation cases, local/full gates, final current-head review, protected merge, and exact merge-head CI pass; no runtime or research trial is added. |
| 4b. Experiment/trial ledger implementation | Blocked by Stage 4a protected merge and exact merge-head CI | Implement the accepted contract in a separate namespace with atomic allocation, append-only events, restart/concurrency/tamper tests, protected-access capability enforcement, campaign completeness, and safe projection. | Behavioral tests prove no silent overwrite, backfilled holdout laundering, failed-before-write loss, retry hiding, prefix truncation, or private projection leak. |
